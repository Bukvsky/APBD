...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(Emp=>Emp.DeptNo.Equals(depts.Where(d=>d.Loc.Equals("CHICAGO"))) ).ToList();
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo).Select(f => new { DeptNo = f.Key, Count = f.Count() });
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e =>
            e.Sal > emps.Where(f => f.DeptNo == e.DeptNo).Average(f => f.Sal)).Select(e => new
        {
            e.EName
        }).ToList();
        //
